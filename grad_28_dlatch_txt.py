...
def simulate_d_latch_from_gates(network, D, EN, prev_Q):
    signals = {'D': D, 'EN': EN, 'nD': 1 - D, 'nEN': 1 - EN, 'Q': prev_Q, 'nQ': 1 - prev_Q}
    for gate in network:
        a = signals[gate['inputs'][0]]
        b = signals[gate['inputs'][1]]
        signals[gate['name']] = GATES[gate['gate']](a, b)
    Q = signals[network[-2]['name']]
    Qn = signals[network[-1]['name']]
    return Q, Qn, signals
...
if best_network:
    ...
    # Save best network as text (human-readable)
    with open("best_d_latch_gates.txt", "w") as txt_file:
        for gate in best_network:
            line = f"{gate['name']}: {gate['gate']}({gate['inputs'][0]}, {gate['inputs'][1]})\n"
            print(line.strip())  # still print to console
            txt_file.write(line)

    # Save best network as JSON (machine-readable for later import)
    with open("best_d_latch_gates.json", "w") as json_file:
        json.dump(best_network, json_file, indent=2)
