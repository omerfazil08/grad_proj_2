# evolution_colab_phase751.py
# Phase 7.5: Dynamic "Rolling Buffer" Genome Sizing
# Fixed: Handles SymPy Objects directly (no parsing needed) to prevent 'strip' errors.
# Fixed: compress_and_lock_genome argument count mismatch.
# Added: convert_to_string_format for VHDL export.
...
import copy
import sympy
from sympy.logic.boolalg import And, Or, Not, Xor, BooleanTrue, BooleanFalse
from sympy.parsing.sympy_parser import parse_expr, standard_transformations, convert_xor

try:
    import simplifier_phase751 as simplifier_phase74
    HAS_SIMPLIFIER = True
except ImportError:
    HAS_SIMPLIFIER = False
    print("⚠️ Warning: simplifier_phase751 not found. Logic compression disabled.")
...
def convert_to_string_format(individual, num_inputs):
    """
    Converts individual to string format expected by VHDL generator.
    """
    str_ind = []
    for i, gate in enumerate(individual):
        ...
        for inp in gate['inputs']:
            if inp < num_inputs: inputs.append(f"A{inp}")
            else: inputs.append(f"G{inp - num_inputs}")
        # e.g., "AND(A0, G1)"
        str_ind.append(f"{gtype}(" + ", ".join(inputs) + ")")
    return str_ind
...
def synthesize_expr_to_gates(expr, num_inputs, gate_offset_start):
    """Rebuilds a SymPy expression into a clean gate list."""
    generated_gates = []
    node_cache = {}
    ...
    def visit(node):
        ...
        if node == BooleanTrue or node == True:
             # Just use A0 OR NOT A0 logic to generate True if needed
             # Or simply return a known True logic if available
             visit(sympy.sympify("A0 | ~A0")) 
             pass 
...
def trace_active_gates(individual, solved_indices, num_inputs, num_outputs):
    ...
    for out_i in solved_indices:
        # Calculate correct index based on genome size
...
def compress_and_lock_genome(best_ind, solved_indices, num_inputs, num_outputs):
    ...
    raw_genome, raw_count, raw_map = defragment_genome_raw(
        best_ind, solved_indices, {}, num_inputs, num_outputs
    )
    ...
    try:
        ...
        # Correctly identify output gate names in the ACTIVE genome
        target_gate_names = []
        for out_i in solved_indices:
            # In the original 'best_ind', the output was at 'orig_idx'
            orig_idx = len(best_ind) - num_outputs + out_i
            # In 'active_genome', it is at 'remap[orig_idx]'
        ...
        try:
            ...
            print(f"   ⏳ Simplifier Error/Timeout. Equation too complex.")
        ...
        for i, out_idx in enumerate(solved_indices):
            ...
            # FIX: Handle both String (old simplifier) and Object (new simplifier) returns
            if isinstance(expr_obj, str):
                transformations = (standard_transformations + (convert_xor,))
                expr_sym = parse_expr(expr_obj, transformations=transformations)
            else:
                expr_sym = expr_obj
...
# --- Main Loop ---
def evolve_phase75(num_inputs, num_outputs, inputs_list, targets_list, cfg):
    ...
    print(f"🏝️ Phase 7.5: Rolling Buffer Sizing.")
    
    # Dynamic Sizing: Initial
    ...
    try:
        for gen in range(cfg.generations):
            ...
            if global_best_raw == max_score_per_col:
                ...
                # FIX: Removed the 4th argument (size) which was causing TypeError
                l_genome, l_count, l_map = compress_and_lock_genome(
                    global_best_ind, indices, num_inputs, num_outputs
                )
