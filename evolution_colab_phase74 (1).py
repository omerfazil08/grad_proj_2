# evolution_colab_phase74.py
# Phase 7.4 Revised: CSE + TIMEOUT PROTECTION
# Fixes the "Freeze" by killing the Simplifier if it takes >5 seconds.

import random
from multiprocessing import Pool, cpu_count, TimeoutError
import sympy
from sympy import cse, Symbol
from sympy.logic.boolalg import And, Or, Not, Xor, Implies, Equivalent, BooleanTrue, BooleanFalse

try:
    import simplifier_phase74
    HAS_SIMPLIFIER = True
except ImportError:
    HAS_SIMPLIFIER = False
    print("⚠️ Warning: simplifier_phase74 not found.")
...
# --- SYNTHESIS & COMPRESSION WITH CSE ---

def synthesize_expr_to_gates(expr, num_inputs, gate_offset_start):
    """Rebuilds gates from SymPy expression using CSE."""
    ...
    def visit(node):
        if node in node_cache: return node_cache[node]
        sig = get_signal_index(node)
        if sig is not None: return sig

        if node == BooleanTrue or node == True:
             idx_not = num_inputs + gate_offset_start + len(generated_gates)
             generated_gates.append({'name': f"G{idx_not}", 'type': 'NOT', 'inputs': [0]})
             idx_or = idx_not + 1
             generated_gates.append({'name': f"G{idx_or}", 'type': 'OR', 'inputs': [0, idx_not]})
             node_cache[node] = idx_or
             return idx_or
        
        if node == BooleanFalse or node == False:
             idx_not = num_inputs + gate_offset_start + len(generated_gates)
             generated_gates.append({'name': f"G{idx_not}", 'type': 'NOT', 'inputs': [0]})
             idx_and = idx_not + 1
             generated_gates.append({'name': f"G{idx_and}", 'type': 'AND', 'inputs': [0, idx_not]})
             node_cache[node] = idx_and
             return idx_and
        ...
        if isinstance(node, Not):
             ...
             node_cache[node] = new_idx
             return new_idx
             
        if isinstance(node, And): op = "AND"
        elif isinstance(node, Or): op = "OR"
        elif isinstance(node, Xor): op = "XOR"
        elif isinstance(node, Equivalent): op = "XNOR"
        elif isinstance(node, Implies): 
            a_idx = visit(node.args[0])
            b_idx = visit(node.args[1])
            not_a = num_inputs + gate_offset_start + len(generated_gates)
            generated_gates.append({'name': f"G{not_a}", 'type': 'NOT', 'inputs': [a_idx]})
            or_idx = not_a + 1
            generated_gates.append({'name': f"G{or_idx}", 'type': 'OR', 'inputs': [not_a, b_idx]})
            node_cache[node] = or_idx
            return or_idx
        ...
        if op:
            ...
            for i in range(1, len(children)):
                ...
                curr = new_idx
            node_cache[node] = curr
            return curr
        
        return 0

    for sym_var, sub_expr in replacements:
        idx = visit(sub_expr)
        node_cache[sym_var] = idx

    final_sig = visit(final_expr)
...
# --- NEW: Wrapper for Timeout ---
def run_simplifier_with_timeout(str_ind, input_names, target_gate_names, timeout_sec=5):
    """Runs simplifier in a separate process to enforce timeout."""
    with Pool(processes=1) as pool:
        res = pool.apply_async(simplifier_phase74.simplify_genome, (str_ind, input_names, target_gate_names))
        try:
            return res.get(timeout=timeout_sec)
        except TimeoutError:
            pool.terminate() # Kill the hanging process
            raise TimeoutError("SymPy took too long")

def compress_and_lock_genome(best_ind, solved_indices, num_inputs, num_gates, num_outputs):
    # 1. Baseline: Raw Defragmentation
    raw_genome, raw_count, raw_map = defragment_genome_raw(
        best_ind, solved_indices, {}, num_inputs, num_gates, num_outputs
    )

    if not HAS_SIMPLIFIER:
        return raw_genome, raw_count, raw_map
    ...
    try:
        ...
        for out_i in solved_indices:
            ...
            if orig_idx in remap:
                ...
                target_gate_names.append("G0")

        # 3. Algebraic Simplification WITH TIMEOUT
        try:
            simplified_map = run_simplifier_with_timeout(str_ind, input_names, target_gate_names, timeout_sec=6)
        except TimeoutError:
            print(f"   ⏳ Simplifier TIMED OUT (>6s). Equation too complex.")
            print(f"   ❌ Using Raw Active Trace ({raw_count} gates).")
            print("-" * 60)
            return raw_genome, raw_count, raw_map
        
        # 4. Re-Synthesize from SymPy Objects
        ...
        for i, out_idx in enumerate(solved_indices):
            ...
            for k in simplified_map.keys():
                if f"Output {out_idx+1}" in k:
                    key_match = k; break
            if not key_match: continue
            
            expr_obj = simplified_map[key_match]
            new_gates, final_sig = synthesize_expr_to_gates(expr_obj, num_inputs, current_gate_offset)
        ...
        # 5. REPORTING
        print("-" * 60)
        print(f"   📊 SIMPLIFICATION REPORT")
        print(f"   1. Raw Active Trace:  {raw_count} gates")
        print(f"   2. SymPy (CSE):       {sim_count} gates")
        
        if sim_count <= raw_count:
            print(f"   ✅ SUCCESS: Using Simplified.")
            print("-" * 60)
            return compressed_gates_all, sim_count, new_output_map
        else:
            print(f"   ❌ REVERT: SymPy bigger. Using Raw.")
            print("-" * 60)
            return raw_genome, raw_count, raw_map
        
    except Exception as e:
        print(f"   ⚠️ Simplifier crashed ({e}). Using Raw.")
        return raw_genome, raw_count, raw_map

def convert_to_string_format(genome, num_inputs):
    lines = []
    for g in genome:
        gtype = g['type']
        inputs = [f"A{x}" if x < num_inputs else f"G{x-num_inputs}" for x in g['inputs']]
        if gtype == 'NOT':
             lines.append(f"{g['name']} = NOT({inputs[0]})")
        elif len(inputs) == 2:
             lines.append(f"{g['name']} = {gtype}({inputs[0]}, {inputs[1]})")
        elif len(inputs) == 3:
             lines.append(f"{g['name']} = {gtype}({inputs[0]}, {inputs[1]}, {inputs[2]})")
    return "\n".join(lines)

# --- Evolve Function ---
def evolve_phase74(num_inputs, num_outputs, inputs_list, targets_list, cfg):
    ...
    print(f"🏝️ Phase 7.4: HSS Islands + Algebraic CSE Compression")
