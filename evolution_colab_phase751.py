...
try:
    ...
    print("⚠️ Warning: simplifier_phase74 not found. Logic compression disabled.")
...
def synthesize_expr_to_gates(expr, num_inputs, gate_offset_start):
    ...
    def visit(node):
        ...
        if node == BooleanTrue or node == True:
             # Generate a "1" (OR NOT(A0), A0) or similar logic? 
             # Simplest: NOT(0) if 0 available? We'll assume NOT(A0 & NOT A0).
             # Actually, easiest is to create logic.
             # Let's assume we can make constant 1.
             # Just use A0 OR NOT A0.
             visit(sympy.sympify("A0 | ~A0")) 
             # But recursion handles it.
             pass 
        ...
        for gen in range(cfg.generations):
            ...
            if global_best_raw == max_score_per_col:
                ...
                l_genome, l_count, l_map = compress_and_lock_genome(
                    global_best_ind, indices, num_inputs, len(global_best_ind), num_outputs
                )
