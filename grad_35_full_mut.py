...
def mutate(individual):
    mutant = []
    # available signals grow as we go down the network (causal circuit)
    available = ['A', 'B', 'nA', 'nB', 'C', 'nC']

    for gate in individual:
        new_gate = gate.copy()

        # gate type mutation (small chance)
        if random.random() < 0.10:
            new_gate['gate'] = random.choice(list(GATES.keys()))

        # input mutation (small, but allowed only from *existing* signals)
        if random.random() < 0.20:
            idx = random.randint(0, 1)
            new_gate['inputs'][idx] = random.choice(available)

        mutant.append(new_gate)
        available.append(new_gate['name'])  # now this is in the correct place
...
sum_simplified = simplify_single_output(best_network, best_network[-2]["name"])
carry_simplified = simplify_single_output(best_network, best_network[-1]["name"])

print("SUM =", sum_simplified)
print("CARRY =", carry_simplified)