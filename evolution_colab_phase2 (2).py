...
def calculate_total_fitness(scores: List[int], solved_mask: List[bool]) -> float:
    """
    Calculates single scalar fitness from component scores.
    Strategy:
    1. Sum scores of unsolved outputs.
    2. Apply HEAVY penalty if a previously solved output (in solved_mask) is broken.
    """
    total = 0
    # Max possible score per output is implicit (assume caller handles scaling if needed)
    # Here we just sum correct bits.
    ...
    for i, score in enumerate(scores):
        ...
        # Penalty logic:
        # If we marked this output as "SOLVED" globally, but this individual
        # has a less-than-perfect score, punish them severely.
        # Note: We need to know max_score per column to know if it's broken.
        # Pass max_score in args? Or just assume dropped score is bad.
        pass 
...
# --- 🆕 PHASE 2 EVOLUTION LOOP: INCREMENTAL GOALS ---
def evolve_colab_phase2(
    ...
    solved_mask = [False] * num_outputs # [True, False, False]
    ...
    if cfg.parallel:
        ...
        # We must re-init pool if solved_mask changes, or pass it dynamically?
        # Passing globals is cleaner via initializer. We'll re-init inside loop if needed
        # or just use a shared memory approach. For simplicity, let's re-init pool ONLY when goals change?
        # Actually, 'solved_mask' is small. We can restart pool if a goal is met.
    ...
    try:
        for gen in range(cfg.generations):
            ...
            if cfg.parallel and pool:
                ...
                # Serial fallback (logic duplicated for safety)
                fitness_scalars = []
                breakdowns = []
            ...
            best_bkd = breakdowns[best_idx] # e.g. [16, 12, 8]
            ...
            # If new goal met, restart pool with new constraints (penalty mask)
            ...
            if gen % cfg.log_every == 0:
                ...
                print(f"Gen {gen:4d} | {status_str} | BestScalar {best_val}")
            ...
            # Ensure the specific solvers for Out0, Out1... are always present
            for i in range(num_outputs):
                if i in hall_of_fame:
                    new_population.append(hall_of_fame[i]) # Inject pure copy
            ...
            while len(new_population) < cfg.pop_size:
                ...
                # Dynamic Mutation: Lower if close to full solution?
                # For Phase 2, standard decay is fine.
    ...
    return best_ind, best_bkd, hall_of_fame
