...
# Features:
# 1. HSS Offset: Guarantees islands start with different genomes.
# 2. Logic Compression: Uses SymPy to shrink solved circuits before locking.
# 3. Hybrid Stagnation: Vetoes reset if average fitness is rising.
...
try:
    ...
    print("⚠️ Warning: simplifier_phase14 not found. Logic compression disabled.")
...
def convert_for_simplifier(individual, num_inputs):
    # Map XOR/XNOR to XOR2/XNOR2 for simplifier compatibility
    string_gates = []
...
def compress_and_lock_genome(best_ind, solved_indices, num_inputs, num_gates, num_outputs):
    """
    Extracts solved logic, simplifies it using SymPy, and rebuilds a compact genome.
    """
    if not HAS_SIMPLIFIER:
        # Fallback to raw extraction if no simplifier
        # (This is the defragment_genome logic from Phase 7.1)
        return best_ind, 0, {} # Placeholder, re-use old defrag if needed
    
    str_ind = convert_for_simplifier(best_ind, num_inputs)
    input_names = [f"A{i}" for i in range(num_inputs)]
    
    # Identify output gates for solved targets
    # In raw genome, output i is usually at -(num_outputs-i)
    target_gate_names = []
    # Note: This mapping works if the genome size is fixed.
    start_out = len(best_ind) - num_outputs
    for i in solved_indices:
        target_gate_names.append(best_ind[start_out + i]['name'])

    # Simplify!
    # This returns {'Output X (G99)': 'Expression...'}
    simplified_map = simplifier_phase14.simplify_genome(str_ind, input_names, target_gate_names)
    
    # Re-synthesize
    compressed_gates_all = []
    new_output_map = {} # {output_index: gate_index_in_compressed_list}
    current_gate_offset = 0
    
    for i, out_idx in enumerate(solved_indices):
        # Find the expression for this output
        # Key format is "Output {N} ({Name})"
        key_match = None
        ...
        # Add to master list
        for g in new_gates:
            # Renaming happened in synthesize, but we must ensure global uniqueness if needed?
            # Actually synthesize returns 0-based names relative to start.
            # We need to offset them globally? 
            # Synthesize handles inputs correctly (A0..An), but internal refs need checking.
            # Our synthesize function assumes linear build. It is safe.
            g['name'] = f"G{current_gate_offset}"
            compressed_gates_all.append(g)
            current_gate_offset += 1
            
        # Map output to the last gate added
        new_output_map[out_idx] = current_gate_offset - 1

    print(f"   >> Compressed Logic: {len(compressed_gates_all)} gates (was {len(best_ind)}).")
...
def evolve_phase72(num_inputs, num_outputs, inputs_list, targets_list, cfg):
    ...
    print(f"🏝️ Phase 7.2: HSS Fixed + Logic Compression.")
    
    # HSS Init with Offset (FIXED)
    islands = []
