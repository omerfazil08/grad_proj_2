# evolution_colab_phase310.py
# Phase 3.10: Bitwise Engine + Defragmentation + SEQUENTIAL FITNESS
# Forces evolution to focus 100% on the next unsolved output bit.
...
def fitness_bitwise(individual, packed_inputs, packed_targets, mask, num_inputs, solved_output_map, current_target_idx):
    """
    SEQUENTIAL FITNESS: Only returns score for 'current_target_idx' + bonus for already solved.
    """
    ...
    # 1. Check Past Solved Outputs (Must stay solved!)
    penalty = 0
    ...
    # If locked, read from map, else read from standard position relative to end
    if current_target_idx in solved_output_map:
        gate_idx = solved_output_map[current_target_idx]
        sig_idx = num_inputs + gate_idx
    else:
        # Dynamic target: We try to use the Last gate available?
        # No, let's stick to the standard convention: Outputs are the LAST N gates.
        # Output i is at index: len - num_targets + i
    ...
    total_score = current_score - penalty
    
    # Also return full breakdown for logging
    full_scores = []
    for i in range(num_targets):
        # Just for logging, calculate all
        # (omitted for speed, we trust the sequential process)
        full_scores.append(0) 
    full_scores[current_target_idx] = current_score
        
    return total_score, full_scores
...
def _eval_wrapper(ind):
    return fitness_bitwise(ind, _PE_data['inputs'], _PE_data['targets'], _PE_data['mask'], _PE_data['n_in'], _PE_data['solved_map'], _PE_data['curr_target'])

# --- DEFRAGMENTATION LOGIC (Same as 3.8) ---
...
def defragment_genome(best_ind, solved_indices, solved_output_map, num_inputs, num_gates, num_outputs):
    ...
    for out_i in range(num_outputs):
        # We only trace outputs up to the current one we just solved
...
def evolve_bitwise(num_inputs, num_outputs, inputs_list, targets_list, cfg):
    ...
    print(f"Bitwise v3.10: Sequential Targeting & Defragmentation.")
    
    population = init_population(num_inputs, cfg)
    solved_mask = [False] * num_outputs
    solved_output_map = {} 
    locked_count = 0
    current_target = 0 # Focus on Output 0 first
        ...
        for gen in range(cfg.generations):
            ...
            if pool:
                ...
                for ind in population:
                    sc, bkd = fitness_bitwise(ind, packed_inputs, packed_targets, mask, num_inputs, solved_output_map, current_target)
                    scalars.append(sc)
                    breakdowns.append(bkd)
            
            best_val = max(scalars)
            best_idx = scalars.index(best_val)
            best_bkd = breakdowns[best_idx]
            
            # Logging (Show score for CURRENT target)
            curr_score = best_bkd[current_target]
            
            if gen % cfg.log_every == 0:
                print(f"Gen {gen:4d} | Target #{current_target+1}: {curr_score}/{max_score_per_col} | Locked={locked_count}")

            # Logic: Did we solve the current target?
            if curr_score == max_score_per_col:
                ...
                # We need to pass list of ALL solved indices
                solved_indices = [i for i, x in enumerate(solved_mask) if x]
                
                new_genome, new_lock, new_map = defragment_genome(
                    best_ind, solved_indices, solved_output_map, num_inputs, cfg.num_gates, num_outputs
                )
                print(f"   >> New Locked Gates: {new_lock}")
                
                # Update World State
                population = [[g.copy() for g in new_genome] for _ in range(cfg.pop_size)]
                locked_count = new_lock
                ...
                # Update Pool for new target
                if pool:
                    pool.close(); pool.join()
                    pool = Pool(processes=cpu_count(), initializer=_init_pool, 
                                initargs=(packed_inputs, packed_targets, mask, num_inputs, solved_output_map, current_target))
                continue # Skip reproduction this gen, start fresh
