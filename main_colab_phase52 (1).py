...
def trace_active_gates(individual, gate_idx, num_inputs):
    """
    Finds all gates that contribute to the output at `gate_idx` (single int).
    """
    active = set()
    stack = [gate_idx] # Initialize stack with the single output gate index
    ...
    while stack:
        ...
        # Add inputs to stack if within bounds
        if g_idx < len(individual):
        ...
        for gen in range(cfg.generations):
            ...
            for i, score in enumerate(best_bkd):
                if score == max_score_per_col and not solved_mask[i]:
                    ...
                    # FIX: Pass single integer index to trace_active_gates
