...
def random_individual(num_gates=4):
    ...
    individual[-2]['output'] = 'sum'
    individual[-1]['output'] = 'carry'
    return individual
...
def evaluate_fitness(individual):
    ...
    for a, b in generate_inputs():
        ...
        sum_gate = next(g for g in individual if g.get('output') == 'sum')
        carry_gate = next(g for g in individual if g.get('output') == 'carry')
        sum_out = signals[sum_gate['name']]
        carry_out = signals[carry_gate['name']]
        if sum_out == half_adder_sum(a, b): score += 1
        if carry_out == half_adder_carry(a, b): score += 1
    return score
...
print("\n🧪 Output Truth Table:")
print(" A B | S C | Target S C")
for a, b in generate_inputs():
    signals = evaluate_network(best_network, a, b)
    s_gate = next(g for g in best_network if g.get("output") == "sum")['name']
    c_gate = next(g for g in best_network if g.get("output") == "carry")['name']
    sum_out = signals[s_gate]
    carry_out = signals[c_gate]
    print(f" {a} {b} | {sum_out} {carry_out} |    {half_adder_sum(a, b)}   {half_adder_carry(a, b)}")

def get_output_gates(network):
    gate_scores = {g['name']: [0, 0] for g in network}  # [sum_score, carry_score]

    for a, b in generate_inputs():
        signals = evaluate_network(network, a, b)
        for g in network:
            output = signals[g['name']]
            if output == half_adder_sum(a, b):
                gate_scores[g['name']][0] += 1
            if output == half_adder_carry(a, b):
                gate_scores[g['name']][1] += 1

    # Choose best matching gates
    best_sum = max(gate_scores, key=lambda g: gate_scores[g][0])
    best_carry = max(gate_scores, key=lambda g: gate_scores[g][1])
    return best_sum, best_carry
...
def generate_structural_vhdl(network, filename="half_adder_custom.vhd"):
    ...
    sum_gate, carry_gate = get_output_gates(network)
