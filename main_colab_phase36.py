...
# Phase 3.6 Driver: Evolution + Online Compression

import matplotlib.pyplot as plt
import numpy as np
import sys
import vhdl_generator_1 
import evolution_colab_phase36 # The Compressing Engine

try:
    from simplifier_phase14 import simplify_genome
    HAS_SIMPLIFIER = True
except ImportError:
    print("WARNING: simplifier_phase14 missing. Compression will NOT work.")
...
def run_5bit_adder_bitwise():
    ...
    cfg = evolution_colab_phase36.BitwiseConfig(
        num_gates=60, 
        pop_size=3000,
        generations=6000, 
        elitism=50,
    ...
    print("🚀 Starting Phase 3.6 (Online Compression)...")
    
    best_ind_raw, best_bkd, hof, history = evolution_colab_phase36.evolve_bitwise(
        n_in, n_out, inputs, targets, cfg
    )
    
    print("Converting genome for export...")
    best_ind_str = evolution_colab_phase36.convert_to_string_format(best_ind_raw, n_in)
