...
# Phase 5 Final: Holistic Hybrid Engine
# Fixes: "Stuck at 512", "TypeError in trace_active_gates", and "Clone Ratchet" bugs.
...
class Phase5Config:
    def __init__(self, 
                 num_gates=40,           # Reduced from 70 to help convergence
                 pop_size=3000,          # Increased to 3000 for diversity
                 generations=5000,       # Total generations (Holistic)
                 elitism=50, 
                 tournament_k=10, 
                 base_mut=0.05, 
                 min_mut=0.005, 
...
# --- Trace Active Logic (Robust Version) ---
def trace_active_gates(individual, gate_idx, num_inputs):
    """Backtracks from a single output gate to find all dependent gates."""
    active = set()
    stack = [gate_idx] # Takes single int
    ...
    while stack:
        ...
        if g_idx < 0 or g_idx >= len(individual): continue # Safety check
...
def fitness_holistic(individual, packed_inputs, packed_targets, mask, num_inputs):
    ...
    for i, target in enumerate(packed_targets):
        # XOR gives differences. Matches = Total - Differences
...
# ==============================================================================
# 4. MAIN EVOLUTION LOOP (Holistic)
# ==============================================================================
def evolve_incremental_phase5(num_inputs, num_outputs, inputs_list, targets_list, cfg):
    ...
    print(f"Phase 5 Revised: Holistic Fitness (Total Generations: {cfg.generations})")
    ...
    try:
        for gen in range(cfg.generations):
            ...
            # Check Solved (Incremental Locking)
            new_solve = False
            ...
            # If new lock, ratchet population
            if new_solve:
            ...
            if locked_indices:
                for i in range(len(new_pop)):
                     for l_idx in locked_indices:
                        new_pop[i][l_idx] = best_ind[l_idx]
