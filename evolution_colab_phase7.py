...
def fitness_bitwise(individual, packed_inputs, packed_targets, mask, num_inputs, solved_output_map, current_target_idx, mode="ACCURACY"):
    ...
    if mode == "NOVELTY":
        ...
        # The main loop will calculate distance
...
def evolve_phase7(num_inputs, num_outputs, inputs_list, targets_list, cfg):
    ...
    try:
        for gen in range(cfg.generations):
            ...
            novelty_archive = [] # Used if in novelty mode
            ...
            for i in range(num_islands):
                ...
                # Track Global Best (Accuracy is always what we care about eventually)
                # Note: Even in Novelty mode, we check if we accidentally solved it
                if evolution_mode == "NOVELTY":
                    # PATCH: Check if the most "novel" individual is actually the solution
                    # We grab the most unique individual (highest scalar distance)
                    local_best_dist = max(scalars)
                    local_idx = scalars.index(local_best_dist)
                    novel_ind = island_pop[local_idx]
                    
                    # Force a quick accuracy check
                    # We only check the current target bit to save time
                    test_signals = evaluate_bitwise(novel_ind, packed_inputs, mask, num_inputs)
                    
                    # Logic copied from fitness_bitwise to get raw score
                    ...
                    # If this "weird" circuit happens to be better, save it!
            ...
            if global_best_raw == max_score_per_col:
                ...
                # Important: Defragment based on the BEST individual found
            ...
            if stagnation_counter > cfg.backtrack_threshold and current_target > 0:
                ...
                # Revert to previous target state
                prev_target = current_target - 1
                
                # We need to UNLOCK the last layer. 
                # Checkpoint stores the state *after* solving prev_target.
                # To re-solve prev_target, we need the state *before* it was solved.
                # Actually, simpler strategy: Just Unlock the last N gates?
                # Better: Revert to the checkpoint of (current_target - 1).
                
                if prev_target in checkpoints:
                    # Load state form checkpoint
                    # BUT wait, if we load checkpoint for target 3, we are effectively saying target 3 is solved.
                    # We want to re-solve target 4. 
                    # If we are stuck on 5, we go back to state where 4 was solved? No, that puts us back here.
                    # We want to go back to state where 3 was solved, and re-solve 4 AND 5.
