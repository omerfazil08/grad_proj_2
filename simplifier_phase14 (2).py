...
# Advanced Simplifier: Forces DNF (AND/OR/NOT) to prevent Parsing Errors.
# Removes XOR (^) from output strings to ensure stability in Phase 7.2.
...
def simplify_genome(best_individual, input_names, output_gate_names):
    ...
    for i, out_gate in enumerate(output_gate_names):
        ...
        # --- SAFE SIMPLIFICATION STRATEGY (DNF Only) ---
        # We force DNF (Disjunctive Normal Form) to avoid XOR (^) symbols.
        # This prevents the parser from confusing ^ (XOR) with ** (Power).
        # DNF format: (A & B) | (C & ~D)
        
        final_expr = simplify_logic(raw_expr, form="dnf")
