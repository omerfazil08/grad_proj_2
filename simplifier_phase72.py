...
# Advanced Simplifier: STRICT DNF MODE.
# Forces AND/OR/NOT logic only. Never outputs XOR (^), preventing parse errors.

from sympy import symbols, And, Or, Not, Xor, simplify_logic
from sympy.logic.boolalg import SOPform, Boolean

print("✅ Loaded simplifier_phase14 (Strict DNF Mode)")
...
def simplify_genome(best_individual, input_names, output_gate_names):
    ...
    for i, out_gate in enumerate(output_gate_names):
        ...
        # --- STRICT FIX ---
        # We force 'dnf' (Disjunctive Normal Form).
        # This guarantees the output uses only &, |, ~
        # It removes all XOR (^) symbols, preventing the Parser Error.
        
        final_expr = simplify_logic(raw_expr, form="dnf", force=True)
