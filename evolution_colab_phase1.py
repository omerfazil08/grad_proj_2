...
class ColabConfig:
    def __init__(
        ...
        # NEW FOR PHASE 1
        self.parallel = parallel        # whether to use parallel fitness
        self.processes = processes      # None => use all available cores
...
def random_gate(num_inputs: int, idx: int, cfg: ColabConfig) -> Dict[str, Any]:
    ...
    if not possible_gate_types:
        ...
        else:  # Truly no sources, return a simple AND gate with dummy inputs
            gate_type = "AND"
            k = 2
            ins = [f"A{i % max(1, num_inputs)}" for i in range(k)]
        ...
        if gate_info["arity"] == "variable":
            # Choose k between min_arity and 3 (max for these variable gates),
            # but not more than available sources
...
def evaluate_network(individual: List[Dict[str, Any]], inputs: Tuple[int, ...]) -> Dict[str, int]:
    ...
    for gate in individual:
        ...
        try:
            ...
            # Should ideally not happen with corrected random_gate,
...
def fitness(individual: List[Dict[str, Any]],
    ...
    if len(individual) < num_outputs:
        # Very low score if not enough gates to represent outputs
        return -1.0 * (cfg.generations + 1)

    # Use the last num_outputs gates as outputs
...
# --- Parallel Fitness Helpers (Phase 1) ---
...
def _pe_init(num_inputs, num_outputs, inputs_set, targets_set, cfg):
    """
    Initializer for worker processes: store shared data in globals.
    """
...
def _pe_fitness(individual):
    """
    Wrapper used by Pool.map: computes fitness for a single individual.
    """
    return fitness(
        individual,
        _PE_num_inputs,
...
def compute_fitnesses_parallel(population: List[List[Dict[str, Any]]],
    ...
    # Avoid ridiculous process counts
    procs = max(1, min(procs, len(population)))
    ...
    with Pool(
        ...
        # chunksize heuristic: spread population roughly evenly across workers
        chunk = max(1, len(population) // (procs * 2) or 1)
        scores = pool.map(_pe_fitness, population, chunksize=chunk)
    return scores
...
def select_parent_tournament(population: List[List[Dict[str, Any]]],
    ...
    tournament_members_indices = random.sample(range(n), k)
...
def evolve_colab_phase0(
    ...
    for gen in range(cfg.generations):
        # === Fitness evaluation (serial or parallel) ===
        if cfg.parallel:
            fitness_scores = compute_fitnesses_parallel(
    ...
    if cfg.parallel:
        final_fitness_scores = compute_fitnesses_parallel(
            population, num_inputs, num_outputs, inputs_set, targets_set, cfg
        )
    else:
