...
def simplify_genome(best_individual, input_names, output_gate_names):
    """
    Main entry point.
    Returns a dictionary: { 'OutputName': 'SimplifiedBooleanString' }
    """
    # 1. Build the full expression map
    expr_map = build_sym_expr_map(best_individual)
    
    results = {}
    
    # 2. Simplify each output
    for i, out_gate in enumerate(output_gate_names):
        if out_gate not in expr_map:
            results[f"Output_{i}"] = "Error: Output gate not found"
            continue
            
        raw_expr = expr_map[out_gate]
        
        # A. First pass: Algebraic simplification
        simple_expr = simplify_logic(raw_expr, form="dnf")
        
        # B. (Optional) Truth-Table Exact Reconstruction (SOPform)
        # This is safer if macros got weird, but simplify_logic is usually enough
        # and faster for large inputs. We stick to simplify_logic for speed.
        
        results[f"Output {i+1} ({out_gate})"] = str(simple_expr)
